...
\[
(u_i > u_{\max} \land e_i > 0) \;\lor\; (u_i < u_{\min} \land e_i < 0) \implies \dot{I}_i = 0
\]
...
\subsection{\vehicle\ Specification of Formal Properties}
\label{app:vehicle-spec}

This appendix provides the complete \vehicle\ specification of the formal properties
evaluated in Section~\ref{sec:vehicle-properties}. 

\paragraph{Initialisation and helper methods (Listing \ref{lst:vehicle-init}).} We treat the network input as an $n$-dimensional real vector
\texttt{State} containing the relevant state components and tracking errors, and the
network output as a $d$-dimensional real vector \texttt{Ctrl} containing the control
commands (system agnostic specification). The predicate \texttt{validState} encodes the bounded input domain
$\mathcal{X}$ by enforcing component-wise lower/upper bounds
(\texttt{minimumInputValues}, \texttt{maximumInputValues}).

\begin{lstlisting}[language={haskell},caption={System-agnostic \vehicle\ specification initialisation.},label={lst:vehicle-init}]
type State = Tensor Real [n] -- n-dim vector of states+errors
type Ctrl  = Tensor Real [d] -- d-dim vector of control outputs

-- Absolute value
abs : Real -> Real
abs r = if r >= 0 then r else -r

-- Input domain (bounded quantification)
minimumInputValues : State
maximumInputValues : State

validState : State -> Bool
validState x = forall i . minimumInputValues ! i <= x ! i <= maximumInputValues ! i
\end{lstlisting}

\paragraph{Network definition and normalisation (Listing \ref{lst:vehicle-network}).}  The function \texttt{normalise} applies the same per-feature affine normalisation used during training, using the vectors named \texttt{meanScalingValues} ($\mu$) and \texttt{stdDevScalingValues} ($\sigma$). The network itself is declared as \texttt{@network ctrlNet : State -> Ctrl}, and \texttt{applyCtrl} wraps the network together with the normalisation. 

\begin{lstlisting}[language={haskell},caption={Normalisation and definition of neural network in \vehicle.},label={lst:vehicle-network}]
-- Normalisation
@parameter
meanScalingValues   : State    -- mu (per-input-feature mean)
@parameter
stdDevScalingValues : State    -- sd (per-input-feature std)

normalise : State -> State
normalise x =
  foreach i . (x ! i - meanScalingValues ! i) / stdDevScalingValues ! i

-- Network definition
@network
ctrlNet : State -> Ctrl

applyCtrl : State -> Ctrl
applyCtrl x = ctrlNet ( normalise x )
\end{lstlisting}


\paragraph{Constants used by properties (Listing \ref{lst:vehicle-constants}).} \texttt{u\_min} and \texttt{u\_max} are vectors describing the control bounds; \texttt{m} is the margin for Property~P1.2; \texttt{epsilon} is the “far-from-reference” threshold for Property~P2; and the vectors \texttt{eps\_eq} and \texttt{u\_quiet} define the equilibrium neighbourhood and allowed small-control region for Property~P3. The elements of \texttt{eps\_eq} and \texttt{u\_quiet} should always be positive to compare with absolute state and control values.


\begin{lstlisting}[language={haskell},caption={\vehicle\ constants used by formal properties.},label={lst:vehicle-constants}]
-- Constants used by properties 
@parameter
u_min : Ctrl    -- actuator lower bounds (vector)
@parameter
u_max : Ctrl    -- actuator upper bounds (vector)

@parameter
m : Real        -- margin for P1.2 (m > 0)

@parameter
epsilon : Real  -- "far from reference" threshold (>0) for P2
@parameter
eps_eq  : State -- "near equilibrium" threshold vector for P3
@parameter
u_quiet : Ctrl  -- quiet-control bound vector for P3
\end{lstlisting}

\paragraph{Formal properties.} Properties P1--P3 are described through Listings \ref{lst:p1}, \ref{lst:p2} and \ref{lst:p3}, respectively. 

\begin{lstlisting}[language={haskell},caption={\vehicle\ specification for Properties P1 and P1.2.},label={lst:p1}]
-- Property P1: Global Saturation Safety
@property
P1_global_saturation : Bool
P1_global_saturation =
  forall x .
    validState x =>
      forall i . u_min ! i <= (applyCtrl x) ! i <= u_max ! i

-- Property P1.2: Saturation Safety with Margin
@property
P1_2_saturation_margin : Bool
P1_2_saturation_margin =
  forall x .
    validState x =>
      forall i . (u_min ! i - m) <= (applyCtrl x) ! i <= (u_max ! i + m)
\end{lstlisting}

Property P2 (Listing \ref{lst:p2}) affects one control channel. Therefore, if the property is to be applied over several control axes, different instances of the same property should be defined. Here, \texttt{e\_idx} and \texttt{u\_idx} are the corresponding state and control output indexes representing the error and control values of the relevant channel. For example, consider Property P2 wants to enforce that if the drone is too far left from the setpoint (the horizontal error $e_x$ is largely negative $e_x \le -\epsilon$), then the control output corresponding to the $x$-axis ($u_x$) should push to the right ($u_x > 0$). This corresponds to the $x$-channel, and \texttt{e\_idx} and \texttt{u\_idx} should be set as the indexes corresponding to $e_x$ and $u_x$ in the state and value vectors.

% While we were not able to verify Property P2, it is worth noting that our NN control outputs are passed through a mixing matrix to obtain individual rotor thrusts (Appendix~\ref{ap:pid}). Therefore, even in the expert PID system we cannot guarantee a direct relationship between the channel-specific outputs and the corresponding thrusts applied to the system dynamics, which could support the unsatisfying verification results obtained for this property.

\begin{lstlisting}[language={haskell},caption={\vehicle\ specification for Property P2.},label={lst:p2}]
-- Property P2: Correct Control Direction at Extremes (example on one channel)
@property
P2_correct_direction : Bool
P2_correct_direction =
  forall x .
    validState x =>
      ( (x ! e_idx) >  epsilon => (applyCtrl x) ! u_idx < 0 ) and
      ( (x ! e_idx) < -epsilon => (applyCtrl x) ! u_idx > 0 )
\end{lstlisting}


\begin{lstlisting}[language={haskell},caption={\vehicle\ specification for Property P3.},label={lst:p3}]
-- Property P3: Quiet Control Near Equilibrium
nearEquilibrium : State -> Bool
nearEquilibrium x = forall i . abs (x ! i) <= eps_eq ! i

@property
P3_quiet_near_equilibrium : Bool
P3_quiet_near_equilibrium =
  forall x .
    validState x and nearEquilibrium x =>
      forall j . abs ((applyCtrl x) ! j) <= u_quiet ! j
\end{lstlisting}

%%%%%%%%%%%%%%% CORA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{CORA property specification and reachability setup}
\label{app:cora}
\lstinputlisting[caption={Closed-loop reachability specification in CORA}]{sections/appendix/cora1.m}
